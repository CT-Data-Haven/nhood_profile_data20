...
acs <- readRDS(file.path("input_data", stringr::str_glue("acs_nhoods_by_city_{yr}.rds"))) %>%
  map(select, -city) %>%
  bind_rows(.id = "city") %>%
...
cdc <- readRDS(file.path("input_data", stringr::str_glue("cdc_health_all_lvls_nhood_{yr}.rds"))) %>%
  mutate(name = ifelse(name %in% dupe_nhoods, paste(name, city), name)) %>%
  select(topic, name, year, indicator = question, value) %>%
  mutate(
    type = factor("m"),
    indicator = indicator %>%
      fct_relabel(janitor::make_clean_names) %>%
      fct_recode(checkup = "annual_checkup", heart_disease = "coronary_heart_disease", asthma = "current_asthma", blood_pressure = "high_blood_pressure", dental = "dental_visit", smoking = "current_smoking", sleep = "sleep_7_hours", life_exp = "life_expectancy", insurance = "health_insurance")
  ) %>%
  inner_join(geos %>% select(city, name), by = "name")
...
out_by_city <- bind_rows(acs, cdc) %>%
  ...
  mutate(topic = case_when(
    ...
    as_factor() %>%
    fct_recode(health_risk_behaviors = "RISKBEH", health_outcomes = "HLTHOUT", prevention = "PREVENT", immigration = "foreign_born", income = "poverty") %>%
...
saveRDS(out_by_city, file.path("output_data", stringr::str_glue("all_nhood_{yr}_acs_health_comb.rds")))